...
import os
def pose_estimation(pose_model,det_model, source,video=False, view_img=False, save_img=False, exist_ok=False):
    
    pose_model = YOLO('models/yolov8l-pose.pt')
    det_model = YOLO('models/yolov8l-seg.pt')
...
def process_folder(model_path, folder_path,video=False, view_img=False, save_img=False, exist_ok=False):
    pose_model = YOLO(model_path)  # Create the model object once here
    det_model = YOLO('models/yolov8l-seg.pt')
    for filename in os.listdir(folder_path):
        file_path = os.path.join(folder_path, filename)
        if os.path.isfile(file_path):
            # pose_estimation(pose_model=pose_model,det_model=det_model, file_path=folder_path, video=video, view_img=view_img, save_img=save_img, exist_ok=exist_ok)
            pose_estimation(pose_model=pose_model, det_model=det_model, source=file_path, video=video, view_img=view_img, save_img=save_img, exist_ok=exist_ok)
...
def main(opt):
    if os.path.isdir(opt.source):
        ...
        if isinstance(opt.model, str):
            pose_model = YOLO(opt.model)  # Instantiate the model for a single file
            det_model = YOLO('models/yolov8l-seg.pt')
        else:
            pose_model = opt.model
            det_model = YOLO('models/yolov8l-seg.pt')
        pose_estimation(pose_model, det_model, opt.source,is_video=opt.video, view_img=opt.view_img, save_img=opt.save_img, exist_ok=opt.exist_ok)
    else:
