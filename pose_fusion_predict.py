...
def pose_estimation(pose_model,seg_model, source,is_video=False, view_img=False, save_img=False, exist_ok=False):
    ...
    if is_video:
        ...
        while cap.isOpened():
            ...
            if success:
                ...
                det_results = seg_model.track(frame, save=True, conf=0.25, iou=0.7, persist=True,device='cpu', classes=[0])
        ...
        det_results = seg_model.predict(image,
                                        # save=True, 
                                        conf=0.25, 
                                        iou=0.7, 
                                        device='cpu', 
...
def process_folder(pose_model,seg_model, folder_path,video=False, view_img=False, save_img=False, exist_ok=False):
    pose_model = YOLO(pose_model)  
    seg_model = YOLO(seg_model)
    for filename in os.listdir(folder_path):
        file_path = os.path.join(folder_path, filename)
        if os.path.isfile(file_path):
            pose_estimation(pose_model=pose_model, seg_model=seg_model, source=file_path, is_video=video, view_img=view_img, save_img=save_img, exist_ok=exist_ok)

def parse_opt():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser()
    parser.add_argument('--pose_model', type=str, default='yolov8l-pose.pt', help='initial weights path for pose model')
    parser.add_argument('--seg_model', type=str, default='yolov8l-seg.pt', help='initial weights path for seg model')
...
def main(opt):
    if os.path.isdir(opt.source):
        process_folder(opt.pose_model,opt.seg_model, opt.source, save_img=opt.save_img, exist_ok=opt.exist_ok)
    elif os.path.isfile(opt.source):
        pose_model = YOLO(opt.pose_model)  # Instantiate the model for a single file
        seg_model = YOLO(opt.seg_model)
        pose_estimation(pose_model, seg_model, source=opt.source, is_video=opt.is_video, view_img=opt.view_img, save_img=opt.save_img, exist_ok=opt.exist_ok)
