...
def parse_opt():
    ...
    # parse if video
    parser.add_argument('--is_video', action='store_true', help='specify if file is video')
...
def main(opt):
    if os.path.isdir(opt.source):
        ...
        if isinstance(opt.model, str):
            ...
            model = opt.model
        pose_estimation(model, opt.source,is_video=opt.is_video, view_img=opt.view_img, save_img=opt.save_img, exist_ok=opt.exist_ok)
    else:
