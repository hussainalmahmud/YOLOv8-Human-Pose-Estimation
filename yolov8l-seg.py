...
def pose_estimation(model, source, view_img=False, save_img=False, exist_ok=False):
    ...
    if is_video_file(source):
        ...
        # print(bboxes.shape)
        # print(bboxes)
        # conf_tensor = bboxes.conf
        # conf_values = conf_tensor.numpy()
        # print(type(conf_values))
        # print(conf_values)
        ...
        # print(type(masks))
        # print(masks)
        # conf_score = results[0].conf
        # print(type(conf_score))
        
        img_annotated = results[0].plot(boxes=False)
        # img_annotated = apply_masks(image.copy(), masks)
        ...
        if save_img:
            save_dir = Path('output_seg') / 'exp'
            save_dir.mkdir(parents=True, exist_ok=True)
            base_filename = Path(source).stem + '_seg'
...
def parse_opt():
    ...
    parser.add_argument('--model', type=str, default='models/yolov8l-seg.pt', help='initial weights path')
