...
def pose_estimation(model, source, view_img=False, save_img=False, exist_ok=False):
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            # img_annotated = pos_results[0].plot(boxes=True)
            ...
            if avg_conf > 0.5:
                # img_annotated_det = det_results[0].plot()
                # img_annotated = img_annotated_det.copy()
                ...
                if pose_results[0].boxes.id is not None:
                    # img_annotated_pose = pose_results[0].plot()
                    # img_annotated = img_annotated_pose.copy()
                    
                    boxes = pose_results[0].boxes.xyxy.cpu()
